--[[
    Enhanced Mirror Movement Script (Revised for Rotation)
    ------------------------------
    Features:
    ...
    - Smoother rotation handling via mirror yaw inversion and interpolation
...
local targetCF0, myCF0 = nil, nil  -- Initial reference CFrames
...
local function CreateGUI()
    ...
    -- Main frame with increased size and rounded corners
    ...
    statusCorner.CornerRadius = UDim.new(1, 0) -- Circle
    ...
    for i, mode in ipairs(modes) do
        ...
        button.BackgroundColor3 = (mode == CONFIG.Mode) 
            and Color3.fromRGB(80, 100, 200)  -- Selected
            or Color3.fromRGB(60, 60, 80)       -- Not selected
        button.Text = mode
...
function UpdateStatus(isActive, targetName)
    ...
    if indicator then
        indicator.BackgroundColor3 = isActive 
...
function CleanupMovers(hrp)
    ...
    if hrp:FindFirstChild("MirrorBodyPosition") then
        ...
        local tween = TweenService:Create(bodyPos, tweenInfo, {
    ...
    if hrp:FindFirstChild("MirrorBodyGyro") then
        ...
        local tweenInfo = TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TweenService:Create(bodyGyro, tweenInfo, {
...
function StartMirroring(target)
    ...
    local lookAt = (CONFIG.Mode == MODES.FOLLOW)
        and (targetHRP.Position + targetHRP.CFrame.LookVector * 10)
    ...
    -- If using Mirror mode, compute the rotation offset so that:
    -- newYaw = -targetYaw + mirrorOffsetAngle reproduces our starting orientation.
    ...
    local tweenInfo = TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local tween = TweenService:Create(bp, tweenInfo, {
        MaxForce = Vector3.new(CONFIG.MaxForce, CONFIG.MaxForce, CONFIG.MaxForce)
    })
    tween:Play()
...
function UpdateMirror(dt, target)
    ...
    local newMyCF
    if CONFIG.Mode == MODES.MIRROR then
        -- Calculate mirrored translation and rotation using the stored baseline
        ...
        -- Override rotation: compute target’s current yaw and set our yaw to its negative plus the offset.
        local pos = newMyCF.Position
        local _, currentTargetYaw, _ = targetHRP.CFrame:ToEulerAnglesYXZ()
        local newYaw = -currentTargetYaw + mirrorOffsetAngle
        newMyCF = CFrame.new(pos) * CFrame.Angles(0, newYaw, 0)
    ...
    local targetPosition = isJumpingOrFalling
        and Vector3.new(newMyCF.Position.X, currentY, newMyCF.Position.Z)
        or newMyCF.Position
